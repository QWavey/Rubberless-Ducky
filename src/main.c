...
static void start_usb_and_wait(void) {
    ...
    usb_device_enable();
    /* Poll until enumerated (g_usb_ready set in cb) or timeout at 3s.
     * This replaces the old blind 200ms spin — we return as soon as
     * Windows has assigned a configuration, not after a fixed delay. */
    uint32_t timeout_ms = 3000;
    while (!g_usb_ready && timeout_ms--) {
        uint32_t start = get_cpu_count();
        while ((get_cpu_count() - start) < 48000) {
            usb_device_task();
        }
    ...
    if (res_mount == FR_OK) {
        ...
        if (res_bin == FR_OK) {
            ...
            if (read_res == FR_OK && br > 0) {
                ...
                // Pre-cache essential sectors before USB takes over PA25 (MISO)
                mc_precache(0);             /* MBR */
                mc_precache(fs.fatbase);    /* FAT start */
                if (fs.fs_type == 3) {      /* FAT32: dirbase is a cluster, root is usually at database */
                    mc_precache(fs.database);
                } else {
                    mc_precache(fs.dirbase); /* FAT12/16: dirbase is a sector */
                }
                ...
                while (pc < word_count) {
                    ...
                    if (word == 0xe801) { // ASSIGNMENT
                        ...
                        /* 100ms is the minimum Windows reliably detects as a USB disconnect.
                         * The original 250ms was unnecessarily long — cutting it saves ~150ms
                         * on every ATTACKMODE switch, which is the hottest path for MSC ready time. */
                        delay_ms(100);
